...
class Paths
{
	...
	public static function clearStoredMemory()
	{
		...
		localTrackedAssets = [];
		localTrackedAssets.resize(localTrackedAssets.length);
	...
	inline static public function modelMaterial(key:String)
		return getModelPath('$key.mtl'); // Or a dedicated folder

	// 3D Model Shader Handling
	inline static public function modelFragmentShader(key:String)
		return getModelPath('shaders/$key', 'frag');

	inline static public function modelVertexShader(key:String)
		return getModelPath('shaders/$key', 'vert');

	inline static public function modelGeometryShader(key:String)
		return getModelPath('shaders/$key', 'geom');
	...
	inline static public function join(paths:Array<String> = null):String
	{
		...
		for (i in 1...paths.length)
		{
			...
			// Add a slash if not already present at the end
			if (!result.endsWith("/") && !result.endsWith("\\"))
				result += "/";

			// Remove leading slash from the part if present
