...
class NdllUtil {
	...
	public static function getFunction(ndll:String, name:String, args:Int):Dynamic {
		...
		if (func == noop) // Check if the function loading failed
			return noop;
	...
	public static function getFunctionFromPath(ndll:String, name:String, args:Int):Dynamic {
		...
		try {
			// Load using the asset path relative to the mods folder, not the full system path
			var func = lime.system.CFFI.load(ndll, name, args);

			if (func == null) {				
				Log.trace('Method ${name} in ndll ${ndll} with ${args} args was not found.', );
				return noop; 
			}
			Log.trace('Successfully loaded function ${name} from ${ndll}.', );
			return func;
		} catch (e:Dynamic) {
			Log.trace('Error loading function ${name} from ndll ${ndll}: ${e}', );
			return noop;
		}

		#else 
		trace('NDLLs are not supported on this platform.', WARNING);
		return noop; // Return noop directly here
		#end
		// This line is unreachable if NDLLS_SUPPORTED is defined, but needed if not
		#if !NDLLS_SUPPORTED
		return noop;
		#end
	}
