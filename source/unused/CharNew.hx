		...
		if(!isAnimateAtlas)
		{
			if (animation != null) {
				try {
					...
					trace('Error playing animation "$AnimName" for character "$curCharacter": $e');
					trace(e.stack);
				}
			}
			else {
				trace('ERROR: animation is null, yes I know what happened here, but it\'s null. for no CRASH!!!');
			}
		}
		else
		{
		...
		if (hasAnimation(AnimName))
		{
			...
			if ( daOffset != null) {
				try {
					...
					trace('ERROR: Animation "$AnimName" offset not found for character "$curCharacter"!');
				}
			} else {
				trace('WARNING: Animation "$AnimName" not found for character "$curCharacter"!');
			}
		}
		//else offset.set(0, 0);
